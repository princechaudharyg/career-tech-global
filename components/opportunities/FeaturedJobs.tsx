...
import { opportunityJobs, type OpportunityJob } from "./opportunityData";
...
function getRecommendedProgram(category: OpportunityJob["category"]): string {
  if (category === "AI & Data") return "Data Science & AI";
  if (category === "Cyber Security") return "Cyber Security Program";
  if (category === "GRC & AI") return "GRC + AI Program";
...
function mapOpportunity(
  ...
  const skills =
    job.skills && job.skills.length > 0 ? job.skills : ["Career opportunity"];
...
function createFallbackJobs(existingJobCount: number): LiveOpportunityJob[] {
  const minimumJobsToShow = 60;
  const requiredFallbackJobs = Math.max(0, minimumJobsToShow - existingJobCount);

  return opportunityJobs.slice(0, requiredFallbackJobs).map((sampleJob) => ({
    ...sampleJob,
...
export default function FeaturedJobs({
  ...
  const [selectedJob, setSelectedJob] = useState<OpportunityJob | null>(null);
  ...
  useEffect(() => {
    async function loadOpportunities() {
      ...
      const liveJobs = ((data || []) as DatabaseOpportunity[]).map(mapOpportunity);
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [filters.keyword, filters.location, filters.jobType, filters.targetRole]);
  ...
  const filteredJobs = useMemo(() => {
    ...
    return allJobs.filter((job) => {
      ...
      const matchesJobType =
        filters.jobType === "All Jobs" || job.type === filters.jobType;
      ...
      return (
        matchesKeyword && matchesLocation && matchesJobType && matchesTargetRole
      );
    });
  }, [allJobs, filters]);
  ...
  const totalCompanies = new Set(allJobs.map((job) => job.company)).size;
          ...
          <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-4 max-w-2xl text-base leading-7 text-slate-500">
                Explore jobs, check role-specific readiness, discover your skill
                gaps, and follow the recommended career path.
              </p>
            ...
            <>
              <div className="mt-12 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
                {visibleJobs.map((job, index) => {
                  const isExternalApplyUrl = job.applyUrl.startsWith("http");
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="mt-5 space-y-2.5 text-sm text-slate-600">
                        ...
                        <div className="flex items-center gap-3">
                          <IconBriefcase size={17} className="text-cyan-600" />
                          <span>{job.type}</span>
                        </div>
                      </div>
                      ...
                      <a
                        href={
                          job.isSample
                            ? job.applyUrl
                            : `/open-opportunities/${job.id}`
                        }
                        target={
                          job.isSample && isExternalApplyUrl
                            ? "_blank"
                            : undefined
                        }
                        rel={
                      ...
                      >
                        {job.isSample ? "Explore Program" : "View & Apply"}
